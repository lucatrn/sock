...
namespace SoLoud
{
	...
	void Soloud::setFilter(handle aVoiceHandle, unsigned int aFilterId, Filter* filter)
	{
		// if (aFilterId >= FILTERS_PER_STREAM) return;

		FOR_ALL_VOICES_PRE
		if (mVoice[ch])
		{
			delete mVoice[ch]->mFilter[aFilterId];
			mVoice[ch]->mFilter[aFilterId] = NULL;

			if (filter)
			{
				mVoice[ch]->mFilter[aFilterId] = filter->createInstance();
			}
		}
		FOR_ALL_VOICES_POST
	}
