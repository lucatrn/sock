...
#define FIXPOINT_FRAC_MASK ((1 << FIXPOINT_FRAC_BITS) - 1)
	...
	void Soloud::mixBus_internal(float *aBuffer, unsigned int aSamplesToRead, unsigned int aBufferSize, float *aScratch, unsigned int aBus, float aSamplerate, unsigned int aChannels)
	{
		...
		for (i = 0; i < mActiveVoiceCount; i++)
		{
			...
			{
				int softPaused = voice->mFlags & AudioSourceInstance::SOFT_PAUSED;
				...
				if (voice->mDelaySamples && !softPaused)
				{
				...
				while (step_fixed != 0 && outofs < aSamplesToRead)
				{
					if (voice->mLeftoverSamples == 0)
					{
						...
						if (softPaused) {
							// Soft paused, set audio to zeroes
							int n = voice->mChannels * SAMPLE_GRANULARITY;
							for (int i = 0; i < n; i++) {
								voice->mResampleData[0]->mData[i] = 0.0f;
							}
						} else {
							// Playing, get audio from source
							int readcount = 0;
				...
				{
					// If have filter with tail, keep alive for a little.
					float maxKeepAlive = 0.0f;
					for (j = 0; j < FILTERS_PER_STREAM; j++)
					{
						if (voice->mFilter[j])
						{
							float keepAlive = voice->mFilter[j]->keepAlive();
							maxKeepAlive = max(maxKeepAlive, keepAlive);
						}
					}

					if (voice->mKeepAlive < maxKeepAlive)
					{
						voice->mKeepAlive += aSamplesToRead / aSamplerate;
					}
					else
					{
