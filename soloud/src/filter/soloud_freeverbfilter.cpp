...
namespace SoLoud
{
	namespace FreeverbImpl
	{
		...
		const float gScalewet = 1;
		const float gScaledry = 1;
		...
		public:
			Revmodel();
			void	mute();
			void	process(float* aSampleData, long aNumSamples, int channelCount);
		...
		void Revmodel::process(float* aSampleData, long aNumSamples, int channelCount)
		{
			if (mDirty)
				update();
			mDirty = 0;
			...
			if (channelCount == 1)
			{
				float* inputLR = aSampleData;

				while (aNumSamples-- > 0)
				{
					float outL, outR, input;
					outL = outR = 0;
					input = (*inputLR) * 2.0f * mGain;
					...
					*inputLR = ((outL * mWet1 + outR * mWet2 + *inputLR * mDry) + outR * mWet1 + outL * mWet2 + *inputLR * mDry) * 0.5f;
	...
	void FreeverbFilterInstance::filter(float* aBuffer, unsigned int aSamples, unsigned int aChannels, float aSamplerate, time aTime)
	{
		...
		mModel->process(aBuffer, aSamples, aChannels);
	}

	float FreeverbFilterInstance::keepAlive()
	{
		// TODO need a guessing algorithm.
		return 3.5f;
	}
