...
namespace SoLoud
{
	EchoFilterInstance::EchoFilterInstance(EchoFilter *aParent)
	{
		...
		mOffset = 0;
		initParams(4);
		mParam[EchoFilter::WET] = aParent->mWet;
	...
	float EchoFilterInstance::keepAlive()
	{
		// Wait for decay volume to reach 0.1.
		//   log(decay, 0.1) * delay
		//   log(0.1)/log(decay) * delay
		//   -2.302585f/log(decay) * delay
		
		float iterations = (-2.302585f / logf(mParam[EchoFilter::DECAY]));
		iterations = max(1.0f, iterations);
		return mParam[EchoFilter::DELAY] * iterations;
	}
